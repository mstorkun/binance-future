...
| 7 | Test coverage too narrow | Yes | Confirmed | Tests increased to 89 plus 3 subtests, but strategy/risk/order tests remain thin. |
...
| 39 | Tests increased | Current test count is 89 plus 3 subtests, but coverage is still not enough for live funds. |
...
2. Add signed listenKey lifecycle helper and websocket runner design for the
   user-data stream; parser is now present.
