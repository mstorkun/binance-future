...
| 7 | Test coverage too narrow | Yes | Confirmed | Tests increased to 91 plus 3 subtests, but strategy/risk/order tests remain thin. |
...
| 39 | Tests increased | Current test count is 91 plus 3 subtests, but coverage is still not enough for live funds. |
...
2. Add websocket runner/event ordering and live-state reconciliation for the
   user-data stream; listenKey helper and parser are now present.
