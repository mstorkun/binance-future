...
import pbo_report
import range_reversion_signal as signal
import risk_metrics
...
class Candidate:
    ...
    @property
    def name(self) -> str:
        reclaim = "REC1" if self.require_reclaim else "REC0"
        daily = "AVD1" if self.avoid_daily_opposite else "AVD0"
        return (
            f"LB{self.lookback}|Z{self.z_min:.1f}|RSI{self.rsi_low:.0f}-{self.rsi_high:.0f}|"
            f"BADX{self.h4_adx_max:.0f}|TV{self.target_vol:.2f}|SL{self.stop_atr_mult:.1f}|"
            f"TP{self.take_profit_atr_mult:.1f}|BW{self.min_band_width_pct:.3f}|{reclaim}|{daily}"
        )
...
class FeatureArrays:
    ...
    close_z: dict[int, np.ndarray]
    low_z: dict[int, np.ndarray]
    high_z: dict[int, np.ndarray]
    band_width_pct: dict[int, np.ndarray]
...
def _log_progress(enabled: bool, message: str) -> None:
    if enabled:
        print(f"[range-reversion] {time.strftime('%Y-%m-%d %H:%M:%S')} {message}", file=sys.stderr, flush=True)
...
def prepare_symbol(symbol: str, df_1h: pd.DataFrame, *, lookbacks: tuple[int, ...] = LOOKBACKS) -> PreparedSymbol:
    df_1h = df_1h.sort_index()
    df_4h = resample_ohlcv(df_1h, "4h")
    df_1d = resample_ohlcv(df_1h, "1D")
    features = signal.build_signal_frame(df_1h=df_1h, df_4h=df_4h, df_1d=df_1d, lookbacks=lookbacks)
...
def generate_candidates(max_candidates: int | None = None) -> list[Candidate]:
    rows = [
        ...
        for z_min in (1.2, 1.5)
        for rsi_low, rsi_high in ((35.0, 65.0), (40.0, 60.0))
        for h4_adx_max in (26.0, 30.0)
        for target_vol in (0.35, 0.50)
        for stop_atr_mult in (1.5,)
        for take_profit_atr_mult in (1.0,)
        for require_reclaim in (False, True)
    ]
...
def build_backtest_data(prepared: dict[str, PreparedSymbol], index: pd.DatetimeIndex) -> BacktestData:
    ...
    for symbol_name, payload in prepared.items():
        ...
        for lookback in LOOKBACKS:
            ...
            mean[lookback] = _float_array(features, f"{prefix}_mean")
            close_z[lookback] = _float_array(features, f"{prefix}_close_z")
            low_z[lookback] = _float_array(features, f"{prefix}_low_z")
            high_z[lookback] = _float_array(features, f"{prefix}_high_z")
            band_width_pct[lookback] = _float_array(features, f"{prefix}_band_width_pct")
        ...
        by_symbol[symbol_name] = FeatureArrays(
            ...
            h1_rsi=_float_array(features, "h1_rsi"),
...
def candidate_signal_sides(data: BacktestData, candidate: Candidate) -> dict[str, np.ndarray]:
    ...
    for symbol_name in data.symbols:
        ...
        regime_ok = (arrays.h4_adx <= float(candidate.h4_adx_max)) & (
            arrays.band_width_pct[lookback] >= float(candidate.min_band_width_pct)
        )
        ...
        if candidate.require_reclaim:
            long_extreme = (arrays.low_z[lookback] <= -float(candidate.z_min)) & (arrays.close_z[lookback] > -float(candidate.z_min))
            short_extreme = (arrays.high_z[lookback] >= float(candidate.z_min)) & (arrays.close_z[lookback] < float(candidate.z_min))
        else:
            long_extreme = arrays.close_z[lookback] <= -float(candidate.z_min)
            short_extreme = arrays.close_z[lookback] >= float(candidate.z_min)
        long_ok = regime_ok & long_daily_ok & (arrays.h1_rsi <= float(candidate.rsi_low)) & long_extreme
        short_ok = regime_ok & short_daily_ok & (arrays.h1_rsi >= float(candidate.rsi_high)) & short_extreme
...
def _metrics(trades: pd.DataFrame, equity: pd.DataFrame, *, start_balance: float) -> dict[str, Any]:
    ...
    *,
    ...
    for offset in range(start, stop):
        ...
        for symbol_name, position in list(positions.items()):
            ...
            if position["side"] == "long":
                if low <= float(position["hard_stop"]):
                    exit_price = float(position["hard_stop"])
                ...
                if high >= float(position["hard_stop"]):
                    exit_price = float(position["hard_stop"])
                    exit_reason = "hard_stop"
                elif low <= float(position["take_profit"]):
                    exit_price = float(position["take_profit"])
                    exit_reason = "mean_revert_tp"
        ...
        for symbol_name in data.symbols:
            ...
            mean = float(arrays.mean[int(candidate.lookback)][offset])
            ...
            stop_distance = float(candidate.stop_atr_mult) * atr
            target_distance = float(candidate.take_profit_atr_mult) * atr
            if side == "long":
                hard_stop = entry - stop_distance
                atr_target = entry + target_distance
                take_profit = min(mean, atr_target) if mean > entry else atr_target
            else:
                hard_stop = entry + stop_distance
                atr_target = entry - target_distance
                take_profit = max(mean, atr_target) if mean < entry else atr_target
...
def run_walk_forward(
    ...
    *,
    ...
    time_stop_bars: int = 24,
    progress: bool = False,
    progress_every_candidates: int = 48,
) -> dict[str, Any]:
    ...
    for fold in ranges:
        ...
        _log_progress(
            progress,
            f"fold {period}/{len(ranges)} train={train_index[0].isoformat()}..{train_index[-1].isoformat()} test={test_index[0].isoformat()}..{test_index[-1].isoformat()}",
        )
        for candidate_number, candidate in enumerate(candidates, start=1):
            candidate_signals = signals_for(candidate)
            _, _, train_metrics = _run_candidate_backtest_arrays(
                backtest_data,
                train_start,
                train_stop,
                candidate,
            ...
            _, _, test_metrics = _run_candidate_backtest_arrays(
                backtest_data,
                test_start,
                test_stop,
                candidate,
            ...
            matrix_rows.append(
                {
                    ...
                    "selected": False,
                    "require_reclaim": bool(candidate.require_reclaim),
                    "lookback": int(candidate.lookback),
                    "z_min": round(float(candidate.z_min), 6),
                }
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Research-only 1h range mean-reversion walk-forward report.")
    parser.add_argument("--symbols", nargs="*", default=list(UNIVERSE))
    parser.add_argument("--years", type=float, default=3.0)
    parser.add_argument("--folds", type=int, default=12)
    parser.add_argument("--train-bars", type=int, default=7200)
    parser.add_argument("--test-bars", type=int, default=900)
    ...
    parser.add_argument("--progress-every-candidates", type=int, default=48)
    parser.add_argument("--out", default="range_reversion_results.csv")
    parser.add_argument("--matrix-out", default="range_reversion_pbo_matrix.csv")
    parser.add_argument("--trades-out", default="range_reversion_trades.csv")
    parser.add_argument("--json-out", default="range_reversion_report.json")
    parser.add_argument("--md-out", default="docs/RANGE_REVERSION_REPORT_2026_05_05.md")
    args = parser.parse_args()

    progress = not args.quiet
    exchange = make_exchange()
    days = int(float(args.years) * 365) + 10
    prepared: dict[str, PreparedSymbol] = {}
    for symbol_name in args.symbols:
        _log_progress(progress, f"fetch {symbol_name} 1h days={days}")
        df_1h = fetch_ohlcv_history(exchange, symbol_name, timeframe="1h", days=days)
        if df_1h.empty:
            _log_progress(progress, f"skip empty symbol={symbol_name}")
            continue
        prepared[symbol_name] = prepare_symbol(symbol_name, df_1h)

    candidates = generate_candidates(args.max_candidates if args.max_candidates > 0 else None)
    _log_progress(progress, f"candidates={len(candidates)}")
    ...
    report["selected"].to_csv(args.out, index=False)
    ...
    json_payload = {
        ...
        "scenarios": report["scenarios"].to_dict(orient="records"),
    }
    Path(args.json_out).write_text(json.dumps(risk_metrics.rounded_nested(json_payload), indent=2, sort_keys=True) + "\n", encoding="utf-8")
    command = " ".join(sys.argv)
    write_markdown(report, args.md_out, command=command)
